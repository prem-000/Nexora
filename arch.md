...
> **Target System:** NEXORA Android Application (`app-debug.apk`)  
...
```mermaid
C4Context
    title System Context Diagram for NEXORA
...
```mermaid
C4Container
    title Container Diagram for NEXORA Application
...
|  - Room Database: AppDatabase (Schema versions 3 -> 4 -> 5 -> 6)        |
...
classDiagram
    ...
    class RouteTable {
        ...
        +invalidateRoutesForNeighbor(neighbor) List~RouteEntry~
        +purgeExpiredRoutes(now) List~RouteEntry~
    }
...
| `destinationPeerId` | `String` | Target peer node identifier (`"ALL"` for broadcast alerts). |
...
| `status` | `PeerStatus` (Enum) | `ONLINE`, `BUSY`, or `OFFLINE`. |
...
ALGORITHM ProcessIncomingPacket(message, senderNeighborAddress, incomingTransportId):
    ...
    5. IF message.destinationPeerId == myDeviceId THEN:
...
sequenceDiagram
    ...
    NodeB->>NodeA: TOPOLOGY_SYNC [subject=NodeB, status=ONLINE, hops=1]
    NodeA->>NodeA: RouteTable.updateRoute(dest=NodeB, nextHop=NodeB, hops=1)

    NodeB->>NodeA: TOPOLOGY_SYNC [subject=NodeC, status=ONLINE, hops=2]
